...
func NewOQueMudaHandler(gemini Simplifier) *OQueMudaHandler {
	return &OQueMudaHandler{gemini: gemini}
}
...
func (h *SimplificarHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[Handler] Enviando para simplificação estruturada...")
	result, err := h.gemini.SimplifyStructured(r.Context(), req.Text)
	if err != nil {
	...
	log.Printf("[Handler] Simplificação estruturada concluída")
...
func (h *OQueMudaHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[Handler] Analisando impacto...")
	result, err := h.gemini.AnalyzeImpact(r.Context(), req.Text)
	if err != nil {
		log.Printf("[Handler] Erro na análise: %v", err)
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(errorResponse{Error: err.Error()})
		return
	...
	log.Printf("[Handler] Análise de impacto concluída")
